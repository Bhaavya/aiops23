...
def get_tr_set(size=None, train_examples=None, batch_size=32, label_list=None ,soft_labels=[], args=None, tokenizer=None,logger=None):
...
def get_eval_set(eval_on, eval_batch_size=8, processor=None, args=None, label_list=None, tokenizer=None, logger=None):
  if eval_on == "dev":
...
'''Evaluation'''
def evaluate(prefix=None, model=None, args=None, eval_on=None, eval_dataloader = None,processor=None, label_list=None, tokenizer=None, logger=None, device=None):
    if eval_dataloader is None:
      eval_dataloader = get_eval_set(eval_on=eval_on, eval_batch_size=args.eval_batch_size,processor=processor, args=args, label_list=label_list, tokenizer=tokenizer, logger=logger)
    ...
    for input_ids, input_mask, segment_ids, label_ids,valid_ids,l_mask in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            loss, logits = model(input_ids, segment_ids, input_mask,labels=label_ids,valid_ids=valid_ids,attention_mask_label=l_mask)
        # print(label_map)
        ...
        for i, label in enumerate(label_ids):
            ...
            for j,m in enumerate(label):
                # print(label_ids[i][j],j,logits[i][j])
                if j == 0:
                    continue
...
def uncertainty_sampling(model_instance, pool, size,args,tokenizer,logger,label_list):
  '''
  ...
  '''
  active_eval_loader = get_tr_set(train_examples=pool, batch_size=1, args=args, tokenizer=tokenizer,logger=logger,label_list=label_list)
  raw_prediction, turncate_list = active_eval(active_eval_loader, model_instance,label_list) # predict, get the softmax output
...
def nte_sampling(model_instance, pool, size, args,tokenizer,logger,label_list):
  active_eval_loader = get_tr_set(train_examples=pool, batch_size=1, args=args, tokenizer=tokenizer,logger=logger,label_list=label_list)
  raw_prediction, turncate_list = active_eval(active_eval_loader, model_instance, label_list) # predict, get the softmax output
...
def qbc_sampling(model_com, pool, n_instance):
  com_pred = []
  active_eval_loader = get_tr_set(train_examples=pool, batch_size=1)
...
def random_sampling(model_instance, input_data, n_instances, args,tokenizer=None,logger=None,label_list=None):
  '''
...
def active_train(data_loader=None, model=None, Epochs=100, soft_loader=None, args=None,num_labels=None,device=None,label_list=None,n_gpu=1,processor=None,tokenizer=None, logger=None):
  ...
  print(num_train_optimization_steps,flush=True)
  ...
  for epoch_idx in trange(int(Epochs), desc="Epoch"):
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Iteration")):
        ...
        nb_tr_steps += 1
        # print(step,args.gradient_accumulation_steps,loss,flush=True)
        ...
        # report = classification_report(y_true, y_pred, digits=4)
        # print(report)
    # report = evaluate('Active Learning', model, args, eval_dataloader=data_loader)
    # print(report)
    if epoch_idx%10 == 0:
      report = evaluate('Active Learning', model, args=args, eval_on='dev', processor=processor, label_list=label_list, tokenizer=tokenizer, logger=logger, device=device)
      print(report)
      report = evaluate('Active Learning', model, args=args, eval_on='test', processor=processor, label_list=label_list, tokenizer=tokenizer, logger=logger, device=device)
      print(report)
...
def active_eval(active_data_loader=None, model=None, label_list =None):
    device = "cuda" 
    model.to(device)
    model.eval()
    eval_loss, eval_accuracy = 0, 0
...
def active_learn(init_flag=None, train_data=None, unlbl_data=None, num_initial=200, 
                 active_policy=None, num_query=5, num_sample=[100, 100, 100, 100, 100], 
                 dev_data=None, fit_only_new_data=False, Epochs=100, prefix='Active', args=None):
  '''
  ...
  report = evaluate('Intialization', model, args, 'test')
  ...
  for idx in range(num_query):
      ...
      report = evaluate('Active Learning', model, args, 'test')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bert_model", type=str, default='jeniya/BERTOverflow')
    parser.add_argument("--data_dir", type=str, default='/home/bhavya2/aiops/pytorch_neural_crf/SeqMix/data/batches_conll/multi_batch_2')
    ...
    if args.do_train:
      train_examples = processor.get_train_examples(os.path.join(args.data_dir, "train.txt"))
      unlbl_examples = processor.get_train_examples(os.path.join(args.data_dir, "unlbl.txt"))
      print(len(train_examples),len(unlbl_examples))
    ...
    model = active_learn(init_flag=False, train_data=train_examples, unlbl_data=unlbl_examples, dev_data=dev_examples, active_policy=active_policy, prefix=args.prefix, Epochs=args.num_train_epochs, args=args)